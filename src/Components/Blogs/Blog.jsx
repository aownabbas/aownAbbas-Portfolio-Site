...
const Blog = ({ date, heading, image, index }) => {
  ...
  return (
    <>
          ...
          <div className="serial__adjust">
            <div className="cont">
              ...
              <h3>
                ...
                <Link to={'#blog'}>{heading}</Link>
              </h3>
            </div>
          </div>
          <div className="opa__thumb imgc">
            <img src={image} alt="img-opa" onClick={() => openLightbox(index)} />
          </div>
        </div>
      ...
      {lightboxOpen && <Lightbox images={blogImages} setLightboxOpen={setLightboxOpen} currentId={currentId} />}
    </>
  );
};
